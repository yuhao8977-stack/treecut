...
DB_PATH = str(Path(__file__).parent.parent / "database" / "material_v3.db")
...
class SmartMatcher:
    ...
    def _lazy_load(self):
        ...
        faiss_path = Path(__file__).parent.parent.parent / "shipin" / "material_faiss.index"
        idmap_path = Path(__file__).parent.parent.parent / "shipin" / "material_faiss_idmap.json"
    ...
    def search(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        main_db = str(Path(__file__).parent.parent.parent / "ai_material_library.db")
            ...
            with sqlite3.connect(main_db) as conn:
                for r in results:
                    ...
                    if not path:
                        continue
                    row = conn.execute(
                        "SELECT AVG(rating) as avg_rating, COUNT(*) as cnt FROM material_feedback WHERE material_path = ?",
                        (path,)
                    ).fetchone()
